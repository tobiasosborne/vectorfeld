...
|---|---|---|
| 2026-06-25 | [p0-remediation-wave](2026-06-25-p0-remediation-wave.md) | **Main remediation campaign** — **21 `3yu` beads shipped** (all data-loss/corruption P0s + every UI P1 + the core-use-case feature **`3yu.2` imported-PDF text editing**: Open PDF/SVG undoable, hit-test→`<text>`, paste-id uniquify, pdf rotation export, ungroup/eraser/directSelect/free-transform/cross-layer-group, Document-Setup, layer hide/lock, ControlBar readout, negative-W/H clamp, ruler guides, bold/italic graft, zoom/pan, and double-click-to-edit imported text re-emitted in the source font), each headed-verified; CLAUDE.md anti-bloat upgrade; ~17 follow-ups raised; 985 tests + golden 11/11. Epic remaining: ONLY P2 cleanup |
