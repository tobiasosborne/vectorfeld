...
## Session log — 2026-04-22 (orientation + design-drop staging)
...
- **bd database is empty on this clone** (`bd stats` → 0). But `.beads/issues.jsonl` has 68 lines including the 30+ `vectorfeld-*` IDs from prior sessions (`ccl`, `byq`, `4w7`, `dcx`, etc.). This is the exact "bd state lost on a fresh device" situation called out in `docs/lessons.md:48` — use the existing jsonl to restore, don't recreate beads from scratch. Restore before doing any bd-tracked work.
...
## Session log — 2026-04-24 (golden suites — gates + milestones, 1→10 grind)
...
Nothing queued from this session — all scoreboard findings are either fixed or their underlying beads are closed with reasons. The scoreboard is now a live regression-detection surface: any future ✗ is a real bug to fix, any ✓→✗ transition is a regression that needs a bead.

### Next work (unchanged — picks up where 2026-04-23 left off)

1. `vectorfeld-byq` — live `mupdf.PDFDocument` handle in DocumentState. Top of the graft-architecture track.
2. `vectorfeld-4w7` — multi-doc UI tabs. Unlocks the tab strip stubbed in Atrium.
3. `vectorfeld-2ss` — Paste in Place (Ctrl+Shift+V).
4. Golden stories 6-10 (the gate-side follow-ons: `u7s`/`00r`/`44a`/`6yf`/`n87`).

### Lessons

- **Turn the headed-Chromium dogfood into a first-class gate, not a script.** `temp/composite-via-playwright.mjs` has been catching bugs for months but only when someone remembered to run it. Promoting that pattern to `npm run golden` made it non-skippable.
